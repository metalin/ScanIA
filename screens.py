...
NUMERIC_TO_SPANISH_SEVERITY = {
    ...
    3: "Low",
    4: "Info",
    # Asegúrese de que todos los valores predichos por su modelo estén aquí.
    # Si su modelo predice "Error" como una clase, agréguelo también.
}

RECOMMENDATIONS_MAP = {
    0: "Actualizar VSFTPD a la última versión sin la vulnerabilidad de backdoor.",
    ...
    3: "Verificar configuraciones seguras de Apache y aplicar parches si es necesario.",
    # ... Incluya todas sus recomendaciones mapeadas a IDs numéricos.
}
...
MASTER_FEATURES_LIST = [
    ...
    # ¡Esta lista debe ser EXACTAMENTE la misma que la que imprimió train_ml_model.py!
...
class DashboardScreen(Screen):
    ...
    def _load_ml_models(self, dt): # 'dt' es el delta time que Clock.schedule_once pasa
        """
        Carga los modelos de Machine Learning (severidad y recomendaciones)
        ...
        try:
            ...
            self.ids.status_label.text = "[color=ffA500]Advertencia: Archivos de modelo ML (severity_model.pkl o recommendation_model.pkl) no encontrados. La clasificación de IA no funcionará. Entrene los modelos primero ejecutando 'python train_ml_model.py'.[/color]"
            print("Advertencia: Archivos de modelo ML no encontrados. La clasificación de IA no funcionará.")
            self.severity_model = None
            self.recommendation_model = None
        except Exception as e:
            self.ids.status_label.text = f"[color=ff0000]Error al cargar modelos ML: {e}. La clasificación de IA podría no funcionar.[/color]"
    ...
    def validar_y_analizar(self):
        ...
        if self.severity_model is None or self.recommendation_model is None:
            self.ids.status_label.text = "[color=ffA500]Advertencia: Los modelos de IA no están cargados. El escaneo continuará sin clasificación inteligente.[/color]"
    ...
    def process_nmap_results(self, host):
        ...
        vulnerabilidades y generar recomendaciones.
        """
        if host not in self.nm.all_hosts(): # Verificar si Nmap encontró el host
            return

        host_data = self.nm[host]
        found_data_for_host = False

        for proto in host_data.all_protocols(): # Iterar sobre protocolos (tcp, udp, etc.)
            ports = host_data[proto].keys() # Obtener los puertos abiertos
            ...
            for port in ports:
                ...
                # Valores por defecto en caso de que el modelo ML no se cargue o falle
                predicted_severity = 'Info'
                predicted_recommendation_text = 'No se encontró recomendación específica o se requiere revisión manual.'
                ...
                # Solo intentar predecir si los modelos están cargados
                if features_for_prediction is not None and self.severity_model and self.recommendation_model:
                    try:
                        ...
                        predicted_recommendation_id = self.recommendation_model.predict(features_for_prediction)[0]
                        predicted_recommendation_text = RECOMMENDATIONS_MAP.get(predicted_recommendation_id, predicted_recommendation_text)

                        # Crear una descripción de vulnerabilidad combinada o basada en ML
                        vuln_description_ml = f"Servicio: {service_name} {product_version}. Puerto: {port}."
                        if 'vulners' in port_data.get('script', {}):
                            vuln_output = port_data['script']['vulners'].strip()
                            # Limitar la salida de vulners para no saturar la UI y el PDF
                            vuln_description_ml += f" Script Vulners: {vuln_output[:100]}..." if len(vuln_output) > 100 else f" Script Vulners: {vuln_output}"
                        elif port_data.get('state') == 'open':
                            vuln_description_ml += " (Puerto abierto detectado)."
                        vuln_description_ml = vuln_description_ml[:250] # Limitar la longitud final

                        # Añadir el resultado con las predicciones del ML
                        self.scan_results.append({
                            'ip': host, 'port': str(port),
                            'service': service_name, 'version': product_version,
                            'vulnerability': vuln_description_ml,
                            'severity': predicted_severity,
                            'recommendation': predicted_recommendation_text
                        })
                        found_data_for_host = True

                    except Exception as e:
                        print(f"Error en la predicción ML para {host}:{port}: {e}")
                        # Fallback si hay un error en la predicción ML
                        self.scan_results.append({
                            'ip': host, 'port': str(port),
                            'service': service_name, 'version': product_version,
                            'vulnerability': f"Error en análisis ML: {str(e)[:150]}",
                            'severity': 'Error',
                            'recommendation': 'Revisión manual necesaria debido a error ML.'
                        })
                        found_data_for_host = True

                elif port_data.get('state') == 'open':
                    # Lógica para puertos abiertos sin vulnerabilidades específicas
                    # O si los modelos ML no se cargaron correctamente
                    self.scan_results.append({
                        'ip': host, 'port': str(port),
                        'service': service_name, 'version': product_version,
                        'vulnerability': f"Servicio {service_name} activo. Sin análisis ML detallado.",
                        'severity': 'Info',
                        'recommendation': 'Verificar necesidad del servicio y configuraciones seguras.'
                    })
                    found_data_for_host = True

        # Lógica para hosts que responden al ping pero sin puertos abiertos detectados (o sin ML)
        if not found_data_for_host and host in self.nm.all_hosts() and self.nm[host].state() == 'up':
            ...
            self.scan_results.append({
                ...
                'severity': 'Info',
                'recommendation': 'Realizar escaneo más profundo o revisión manual si se sospecha de actividad.'
            })
    ...
    def finish_scan(self, dt=None):
        ...
        if self.scan_results:
            ...
            if med > 0: vuln_counts_display.append(f"[color=ffff00]{med} {SPANISH_SEVERITY['Medium']}s[/color]")
            if low > 0: vuln_counts_display.append(f"[color=00cc00]{low} {SPANISH_SEVERITY['Low']}s[/color]")
            if info > 0: vuln_counts_display.append(f"[color=6495ED]{info} {SPANISH_SEVERITY['Info']}s[/color]")
    ...
    def display_results(self):
        ...
        # Eliminadas las referencias a _background_rect_instruction ya que la Image fue quitada del KV.
        # if hasattr(results_container, '_background_rect_instruction') and results_container._background_rect_instruction:
        #     results_container.canvas.before.remove(results_container._background_rect_instruction)
        #     results_container._background_rect_instruction = None # Clear the reference

        if not self.scan_results:
            results_container.add_widget(Label(text="No se encontraron resultados para mostrar.",
                                            size_hint_y=None, height=dp(40)))
            # No se llama a show_results_background_image porque ya no es un Image widget en KV
            return
    ...
    # Eliminadas las funciones show_results_background_image y _update_results_background_rect
    # ya que el widget Image fue eliminado del KV.
    # def show_results_background_image(self):
    #     """Adds the background image to the results container."""
    #     results_container = self.ids.results_container
    #     # Only add if it's not already there
    #     if not hasattr(results_container, '_background_rect_instruction') or not results_container._background_rect_instruction:
    #         with results_container.canvas.before:
    #             Color(1, 1, 1, 1) # Full opacity for the image
    #             # Store the instruction reference to remove it later
    #             results_container._background_rect_instruction = Rectangle(
    #                # source='Fondo.jpg',
    #                # size=(dp(200), dp(200)),
    #                 #pos=(results_container.center_x - dp(100), results_container.center_y - dp(100))
    #             )
    #         # Bind the image's position and size to the container's for proper centering
    #         results_container.bind(pos=self._update_results_background_rect, size=self._update_results_background_rect)

    # def _update_results_background_rect(self, instance, value):
    #     """Updates the position and size of the background image."""
    #     if hasattr(instance, '_background_rect_instruction') and instance._background_rect_instruction:
    #         instance._background_rect_instruction.pos = (instance.center_x - dp(100), instance.center_y - dp(100))
            ...
            try:
                ...
                for i, severity_key_for_plot in enumerate(['Critical', 'High', 'Medium', 'Low', 'Info']):
                    ...
                    points = [
                        ...
                        (i + bar_width / 2, 0),                       # Esquina inferior derecha
                        (i - bar_width / 2, 0)                        # Vuelve a la esquina inferior izquierda para cerrar el polígono
                    ]
                    
                    # Se usa MeshLinePlot con un color relleno para simular barras
                    # Nota: MeshLinePlot dibuja líneas, no rellena. Para barras llenas necesitarías un GraphicInstruction más complejo como Rectangle.
                    # Sin embargo, para una representación simple tipo barra con líneas, esto puede funcionar.
                    # Si buscas barras llenas, se requeriría un enfoque con `kivy.graphics.Rectangle`
                    # directamente en el canvas del widget, lo cual es más complejo para Graph.
                    # Para este Graph, simularé barras usando un MeshLinePlot que dibuja el contorno.
        ...
        try:
            ...
            if not grouped_by_host:
                ...
                for host_ip in sorted_host_ips:
                    ...
                    host_vuln_data = [['Puerto', 'Servicio', 'Versión', 'Vulnerabilidad', 'Severidad', 'Recomendación']]
                    ...
                    for vuln in sorted_vulnerabilities:
                        ...
                        host_vuln_data.append([
                            ...
                            vuln.get('recommendation', 'N/A') # Obtener la recomendación (usar .get para seguridad)
                        ])
                    ...
                    if len(host_vuln_data) > 1: # If more than just the header
                        ...
                        host_table = Table(host_vuln_data, colWidths=[0.6*inch, 1.0*inch, 1.0*inch, 2.0*inch, 0.8*inch, 1.5*inch])
